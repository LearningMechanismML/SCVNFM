...
for gogo = 1:ExampleNum
    ...
    DataNum = size(data,1);                     
    sorted_target = sort(data(:,InputNeuronsNum+1));  
    label(1,1)=sorted_target(1,1);
    j=1;
    for i = 2:DataNum                           
        if sorted_target(i,1) ~= label(1,j)     
            j=j+1;                              
            label(1,j) = sorted_target(i,1);    
        end
    end
    ClassNum=j;                                 
    OutputNeuronsNum=ClassNum;                  
    rowrank = randperm(DataNum);                
    data = data(rowrank,:);
    AttributeData = data(:,1:InputNeuronsNum);  
    TableData = data(:,InputNeuronsNum+1);      
    TrainNum = round(0.7*DataNum);                     
    TestNum = DataNum - TrainNum;               
    Train_Atttibute = AttributeData(1:TrainNum,:);              
    Train_Table = TableData(1:TrainNum,:);                      
    Test_Atttibute = AttributeData((TrainNum+1):DataNum,:);     
    Test_Table = TableData((TrainNum+1):DataNum,:);             
